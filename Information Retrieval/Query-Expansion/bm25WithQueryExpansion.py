...
class BM25:
	...
	for line in rel_file:
		...
		if k in rel_dict:
			...
			rel_dict[k].append(temp)	
	print rel_dict
	fr1= open("invertedindex_unigram.txt","rb")
	uniIndex=pickle.load(fr1)
	#fr2= open('doctoID.p’,’r’)
	doc=pickle.load(open('doctoID.txt','rb'))
	#fr3= open('tokenCount.p','r')
	docLen=pickle.load(open('tokenCount.txt','rb'))
	#fr4= open('tokensInDoc.p','r')
	dcdt=pickle.load(open('tokensInDoc.txt','rb'))
	length=len(doc)
	avdl=0
	k1=1.2
	k2=100
